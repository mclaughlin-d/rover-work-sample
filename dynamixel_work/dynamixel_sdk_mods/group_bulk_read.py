...
class GroupBulkRead:
    def __init__(self, port, ph):
        ...
        self.last_result_dict = (
            {}
        )  # stores whether the last read was successful for each ID
    ...
    def addParam(self, dxl_id, start_address, data_length):
        ...
        self.last_result_dict[dxl_id] = False  # never read previously, so set to false
    ...
    def rxPacket(self):
        ...
        for dxl_id in self.data_dict:
            ...
            if result == COMM_SUCCESS:
                self.last_result_dict[dxl_id] = True  # read was successful
            else:
                self.last_result_dict[dxl_id] = False
    ...
    def isAvailable(self, dxl_id, address, data_length):
        if (
            self.last_result_dict[dxl_id] is False or dxl_id not in self.data_dict
        ):  # modified to check self.last_result_dict for specific ID
            return False
